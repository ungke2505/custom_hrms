...
const LateEntryPermission = () => {
  const [employee, setEmployee] = useState("");
  const [employeeName, setEmployeeName] = useState(""); // 🔹 state untuk nama lengkap
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        if (!user || user === "Guest") {
          setError("Kamu harus login untuk mengirim izin terlambat.");
          return;
        }
        ...
        const empRes = await fetch("/api/method/frappe.client.get_list", {
          ...
          body: JSON.stringify({
            ...
            fields: ["name", "employee_name"], // 🔹 ambil juga nama lengkap
            limit_page_length: 1,
          }),
        });
        ...
        if (emp) {
          ...
          setEmployeeName(emp.employee_name || "—"); // 🔹 fallback jika kosong
        } else {
  ...
  const handleSubmit = async () => {
    if (!reason.trim()) {
      setError("Harap isi alasan keterlambatan terlebih dahulu.");
      return;
    }
    ...
    if (!window.confirm("Yakin ingin mengirim izin terlambat?")) return;
    ...
    try {
      ...
      try {
        const overlayText = `Izin Terlambat • ${new Date().toLocaleString("id-ID")}`;
      ...
      const res = await fetch(
        "/api/method/custom_hrms.api.late_entry_permission.create_late_entry",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
...
export default LateEntryPermission;
